function solve() {
    ...
    quickCheckButton.addEventListener('click', () => {
        ...
        rows.forEach(row => {
            const nums = Array.from(row.querySelectorAll('td input')).map(input => Number(input.value));
            nums.forEach((num, i) => columns[i].push(num));

            if (sumNumbers(...nums) !== 6) correctSum = false;
        });
