...
function solution() {
    ...
    const createUserElement = (data) => {
        const labels = ['Full Name: ', 'Email: ', 'Phone Number: ', 'Address: ', 'Postal Code: '];
        labels.forEach((label, index) => {
            const li = createElement({
                tag: 'li',
                textContent: `${label}${data[index].value}`
            });
            agency.preView.appendChild(li);
        });
    };
    ...
    const editBtnFunctionality = () => {
        const data = Object.values(inputFields);
        Array.from(document.querySelectorAll('#infoPreview li')).forEach((x, index) => {
